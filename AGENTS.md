...
**Auth:** Bearer JWT — must be set before Pi starts. Generate via `POST /api/auth/login` with the bot's service-account credentials (see `.env` on server, `BIGBASE_EMAIL`/`BIGBASE_PASSWORD` — never hardcode these).
...
**Auth:** User API Key (NRAK-...) — configured in `.pi/mcp.json`. Requires Pi restart to appear.
...
Required: `TELEGRAM_TOKEN`, `GRUPO_CHAT_ID`, `ADMIN_IDS`, `BIGBASE_URL`, `BIGBASE_EMAIL`, `BIGBASE_PASSWORD`, `APIFOOTBALL_KEY`, `APIFOOTBALL_LEAGUE_ID` (28 — Copa 2026, NOT 1), `RESULTS_PROVIDER` (apifootball). Never hardcode these values in code, docs, or specs — read from `.env` only.
