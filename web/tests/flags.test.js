...
import { flag } from "../src/scoring.js";
...
test("coverage: no tournament team falls back to the white flag", () => {
  ...
  assert.deepEqual(
    missing,
    [],
    `countries with no flag: ${missing.join(", ")}`
  );
});
...
test("bosnia & herzegovina (ampersand form from API) resolves to real flag", () => {
  assert.equal(
    ...
    "🇧🇦",
    "ampersand form should match"
  );
  assert.equal(
    flag("Bosnia and Herzegovina"),
    "🇧🇦",
    "and form should still match"
  );
});
...
// Portuguese names used in manually-created knockout matches (BigBase seed fallback)
const PT_NAMES = [
  ["Inglaterra", "🏴󠁧󠁢󠁥󠁮󠁧󠁿"],
  ["França", "🇫🇷"],
  ["Espanha", "🇪🇸"],
...
test("Portuguese team names from manual knockout seeds resolve to flags", () => {
  for (const [pt, expected] of PT_NAMES) {
    assert.equal(flag(pt), expected, `${pt} should resolve to ${expected}`);
  }
});
