...
// Mirrors bolao/scoring.py FASE_PONTOS — must stay in sync.
const FASE_PONTOS = {
  R32: [5, 2],
  R16: [10, 5],
  QF: [15, 10],
...
export function calcPontos(pa, pb, ra, rb, matchId = "") {
  ...
  if (pa === ra && pb === rb) return ptsExato;
  const s = (x, y) => (x > y ? 1 : x < y ? -1 : 0);
  return s(pa, pb) === s(ra, rb) ? ptsVencedor : 0;
}
...
export function calcRanking(jogos, palpites, participantes) {
  const ativos = participantes.filter(
    (p) => p.ativo !== false && Number(p.telegram_id) > 0
  );
  const nomes = Object.fromEntries(
    ativos.map((p) => [Number(p.telegram_id), p.nome || "?"])
  );
  const enc = {};
  ...
  for (const p of palpites) {
    ...
    const pc = Number(p.gols_casa),
      pf = Number(p.gols_fora);
    const pts = calcPontos(pc, pf, ra, rb, p.match_id);
    const e =
      acc[tid] ||
      (acc[tid] = {
        telegram_id: tid,
